...
### Fixes
- **`scan_state` / `token_usage` tables missing after upgrade from old .deb**: Users whose `.deb` install had already recorded `1.3.0` in `schema_version` — but with the original schema that used `consumption_samples` / `consumption_ingest_state` — never received the real `token_usage` and `scan_state` tables. The token scanner consequently logged *"Token scan failed: no such table: scan_state"* every 5 minutes. A new `2.0.0` migration (disk file + embedded) creates both tables with `IF NOT EXISTS`, so existing installs pick it up on next startup and fresh installs stay unaffected (idempotent DDL).
- **SSRF guard missed several IPv6 forms**: `isLoopbackOrPrivate()` compared against the raw `URL.hostname` string — which Node.js wraps in brackets for IPv6 (e.g. `[::1]` not `::1`) — so IPv6 loopback was never blocked. Also added: unique-local (`fc00::/7`, fc/fd prefix), link-local (`fe80::/10`, fe[89ab] prefix), and IPv4-mapped addresses in Node's hex-normalised form (`::ffff:7f00:1` for 127.x, `::ffff:a__:` for 10.x, etc.). Brackets are now stripped before the hostname is tested.
- **HTTP provider endpoint built by string concatenation**: If the base URL had a query string (e.g. `https://host/api?v=2`), the old `url + '/search'` placed `/search` inside the query component, producing a 404. The endpoint is now constructed via the URL object (`u.pathname`, `u.search = ''`, `u.hash = ''`) so the SSRF-checked URL and the fetch target are always in sync.
- **`McpKnowledgeProvider.dispose()` race leaked a live client**: If `dispose()` was called while `getClient()` was mid-connect, the `connecting` promise was not cleared, leaving a connected `Client` instance alive after the provider was logically dead. A `disposed` flag is now checked immediately on entry to `getClient()` (throws) and again after `client.connect()` completes (closes and throws), and `dispose()` nulls `this.connecting` before returning.
- **Secrets added post-startup were invisible to the running server**: `EnvSecretStore` reads `process.env` at request time, but the sidecar only injected keychain secrets into Node's env at spawn — any provider added after startup had no secret. A new `POST /api/providers/:id/secret` endpoint accepts the plaintext value from the dashboard UI and writes it directly into `process.env`, making it available immediately without a restart. The UI's `save()` path now calls this endpoint before writing to the keychain.
- **Keychain entry outlived `providers.json` entry on delete**: The old delete flow removed the provider from `providers.json` first; if the keychain call then failed, the secret was orphaned (no longer associated with any provider id and thus unreachable for cleanup). Reversed: keychain delete runs first (best-effort), then `providers.json` is updated, so the id is always present for `cleanup_provider_secrets` to find.
- **Malformed `providers.json` was silently swallowed**: Parse/validation errors in `loadProviders()` and `reloadProviders()` were caught and discarded, making misconfigured provider files invisible. Both now log to stderr with `[CogniStore] Failed to load providers.json: <message>`.
- **Sidecar identity token used weak entropy**: The 16-character token was derived from `process.pid + Date.now().toString(36)` — predictable under pid reuse or fast startup. It is now generated with `getrandom::fill` (16 CSPRNG bytes → 32-char hex) via the `getrandom 0.3` crate.

### Tests
- Added 7 new provider tests: IPv6 SSRF (loopback `[::1]`, unique-local `[fc00::1]`, link-local `[fe80::1]`, IPv4-mapped `[::ffff:127.0.0.1]`), URL-object endpoint construction, `dispose()` called mid-connect (no leaked client), and `dispose()` before any connection (no throw).
